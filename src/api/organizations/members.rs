...
impl OrganizationMembersApi {
    ...
    pub fn assign_role(
        ...
        AssignMemberRoleBuilder::new(self.client.clone(), organization_id, membership_id, role_id)
    }
    ...
    pub fn remove_role(
        ...
        RemoveMemberRoleBuilder::new(self.client.clone(), organization_id, membership_id, role_id)
    }
}
...
/// Builder for assigning a role to an organization member
    ...
    pub async fn send(self) -> Result<()> {
        ...
        if status.is_success() {
            ...
            Err(Error::api_from_text(
                status,
                format!(
                    "Failed to assign role {} to member {} in organization {}",
                    self.role_id, self.membership_id, self.organization_id
                ),
                &error_body,
            ))
        }
...
/// Builder for removing a role from an organization member
    ...
    pub async fn send(self) -> Result<()> {
        ...
        if status.is_success() {
            ...
            Err(Error::api_from_text(
                status,
                format!(
                    "Failed to remove role {} from member {} in organization {}",
                    self.role_id, self.membership_id, self.organization_id
                ),
                &error_body,
            ))
